...
# --- BANCOS DE DADOS ---
# Caminhos atualizados para a pasta 'db'
DB_FOLDER = 'db'
DB_PRODUCAO = os.path.join(DB_FOLDER, 'producao_cirurgica.db')
DB_MEDICOS = os.path.join(DB_FOLDER, 'medicos.db')  # Correção: Aponta para db/medicos.db
...
def load_user(user_id):
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT id, nome_completo, email, nivel_acesso, primeiro_acesso FROM usuarios WHERE id = ?",
                   (user_id,))
    row = cursor.fetchone()
    conn.close()
    if row:
...
def get_db_connection(db_path):
    # Verifica se o arquivo existe para evitar erros silenciosos
...
def get_medicos_conn(): return get_db_connection(DB_MEDICOS)  # Usa DB_MEDICOS (db/medicos.db)
...
# --- MÓDULO EMPRESAS (Lógica de Extração PDF) ---

def extrair_dados_pdf(filepath):
    """Extrai dados do contrato PDF (Razão Social, CNPJ, Objeto, Data, Tabelas de Escopo)"""
    dados = {
        'razao_social': '',
        'cnpj': '',
        'objeto': '',
                ...
                for table in tables:
                    for row in table:
                        ...
                        if any(k in row_str for k in
                               ['consulta', 'exame', 'cirurgia', 'plantão', 'procedimento']) and any(
                                char.isdigit() for char in row_str):
                            clean_row = [str(cell).replace('\n', ' ').strip() if cell else '' for cell in row]
                            dados['escopo'].append(clean_row)

            # Regex
            match_rs = re.search(r'Pelo presente,\s*(.*?),\s*inscrito no CNPJ', full_text, re.IGNORECASE | re.DOTALL)
            if match_rs:
                dados['razao_social'] = match_rs.group(1).replace('\n', ' ').strip().upper()

            match_cnpj = re.search(r'\d{2}\.\d{3}\.\d{3}/\d{4}-\d{2}', full_text)
            if match_cnpj:
                dados['cnpj'] = match_cnpj.group(0)

            match_obj = re.search(r'Constitui objeto do presente contrato\s*(.*?)\.', full_text,
                                  re.IGNORECASE | re.DOTALL)
            if match_obj:
                dados['objeto'] = match_obj.group(1).replace('\n', ' ').strip()

            match_dates = re.findall(r'São Paulo,\s*\d{1,2}\s*de\s*[A-Za-zç]+\s*de\s*\d{4}', full_text, re.IGNORECASE)
            if match_dates:
                dados['data'] = match_dates[-1]
...
def empresas_salvar():
    try:
        ...
        cursor.execute("""
                       INSERT INTO empresas (razao_social, cnpj, objeto_contrato, data_celebracao, escopo_json,
                                             arquivo_contrato)
                       VALUES (?, ?, ?, ?, ?, ?)
                       """, (
...
# --- API MÉDICOS E ESPECIALIDADES (ATUALIZADO) ---

@app.route('/api/medicos', methods=['GET'])
@login_required
def get_medicos():
    # Agora usa get_medicos_conn() que aponta para db/medicos.db
...
def get_especialidades_amec():
    conn = get_medicos_conn()  # Correção: usa db/medicos.db
    try:
        cursor = conn.cursor()
...
# API de busca de procedimentos (Mantida)
...
def search_procedimento():
    ...
    for row in cursor.fetchall():
        proc = dict(row)
        cursor.execute("SELECT tipo, especialidade FROM producao WHERE codigo_sigtap = ? LIMIT 1",
                       (proc['codigo_sigtap'],))
